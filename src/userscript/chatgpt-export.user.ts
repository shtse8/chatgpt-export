...
// @version      __VERSION__
// @description  Export all your ChatGPT conversations to JSON
// @author       Kyle Tse
...
function createWidget(): void {
  ...
  minimizeBtn.addEventListener('click', () => {
    ...
    widget.style.minWidth = minimized ? 'auto' : '220px'
  })
  ...
  const updateUI = (p: Progress): void => {
    ...
    const statusMap: Record<string, string> = {
      authenticating: '🔑 Auth...',
      listing: '📂 Listing...',
      downloading: `⬇️ ${p.downloaded}/${p.total} (${p.rate.toFixed(0)}/min)`,
